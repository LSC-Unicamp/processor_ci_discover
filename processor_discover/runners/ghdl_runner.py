...
def _search_repo_for_declaration(
    repo_root: str, symbol_name: str, symbol_type: str, repo_name: str = None
) -> List[str]:
    """
    ...
    pattern = re.compile(
        rf"^\s*{symbol_type}\s+{re.escape(symbol_name)}\s+is\b",
        flags=re.IGNORECASE | re.MULTILINE,
    )
        ...
        for file in files:
            ...
            try:
                with open(full_path, "r", encoding="utf-8", errors="ignore") as fh:
                    content = (
                        fh.read()
                    )  # Read entire file to ensure we find all symbol declarations
                    if pattern.search(content):
                        # Convert to relative path
                        rel_path = os.path.relpath(full_path, repo_root)
                        # Normalize path
                        rel_path = (
                            _normalize_file_path(rel_path, repo_name)
                            if repo_name
                            else rel_path
                        )
                        candidates.append(rel_path)
                        print_yellow(
                            f"[GHDL-INCREMENTAL] Found {symbol_type} '{symbol_name}' in: {rel_path}"
                        )
            except Exception:
                continue
...
def _find_file_declaring_entity(
    repo_root: str,
    entity_name: str,
    ...
    for mod_name, file_path in modules:
        if mod_name.lower() == entity_lower:
            normalized_path = (
                _normalize_file_path(file_path, repo_name) if repo_name else file_path
            )
            candidates.append(normalized_path)

    # If not found in modules, search entire repository
    if not candidates:
        candidates = _search_repo_for_declaration(
            repo_root, entity_name, "entity", repo_name
        )
...
def _find_file_declaring_package(
    ...
    candidates = _search_repo_for_declaration(
        repo_root, package_name, "package", repo_name
    )
...
def _parse_missing_entities_with_context(log_text: str) -> List[Tuple[str, str]]:
    ...
    pattern = (
        r'([^\s:]+\.vhdl?):\d+:\d+:.*?no\s+declaration\s+for\s+["\']([^"\']+)["\']'
    )
...
def _reorder_by_dependencies(
    files: List[str], log_text: str, repo_root: str
) -> List[str]:
    """
    ...
    print_yellow(
        f"[GHDL-INCREMENTAL] Found dependencies - packages: {len(missing_pkg_deps)}, entities: {len(missing_ent_deps)}"
    )
    ...
    must_come_before: Dict[str, Set[str]] = (
        {}
    )  # provider_file -> set of files that need it
    ...
    for error_file, missing_symbol in missing_pkg_deps:
        ...
        if provider_file and provider_file != error_file_normalized:
            ...
            print_green(
                f"[GHDL-INCREMENTAL] Package dependency: {provider_file} must come before {error_file_normalized} (provides {missing_symbol})"
            )
    ...
    for error_file, missing_symbol in missing_ent_deps:
        ...
        if provider_file and provider_file != error_file_normalized:
            ...
            must_come_before[provider_file].add(error_file_normalized)
            print_green(
                f"[GHDL-INCREMENTAL] Entity dependency: {provider_file} must come before {error_file_normalized} (provides {missing_symbol})"
            )

    if not must_come_before:
        print_yellow(
            "[GHDL-INCREMENTAL] No actionable dependencies found for reordering"
        )
        return files
    ...
    for provider, dependents in must_come_before.items():
        ...
        if earliest_needed < provider_idx:
            ...
            print_yellow(
                f"[GHDL-INCREMENTAL] Reordered: moved {provider} before {len(dependents)} dependent(s)"
            )
...
def _ghdl_clean_work(repo_root: str, workdir: str, library_name: str = "work") -> None:
    ...
    if os.path.exists(library_file):
        try:
            ...
            print_yellow(
                f"[GHDL-INCREMENTAL] Warning: Could not clean library file: {e}"
            )
            ...
            with open(full_path, "r", encoding="utf-8", errors="ignore") as fh:
                ...
                for pattern in vendor_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        print_yellow(
                            f"[GHDL-INCREMENTAL] Detected vendor-specific library in {file}, excluding from compilation"
                        )
        ...
        try:
            with open(full_path, "r", encoding="utf-8", errors="ignore") as fh:
                ...
                for pattern in synopsys_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        print_yellow(
                            f"[GHDL-INCREMENTAL] Detected Synopsys package usage in {file}"
                        )
                        return True
        except Exception:
            continue
...
def _validation_flags(
    ghdl_extra_flags: List[str] = None, files: List[str] = None, repo_root: str = None
) -> List[str]:
    """
    ...
    has_warn_binding = any(
        (f.startswith("--warn-error=") and "binding" in f) for f in base
    )
    ...
    if not has_synopsys and files and repo_root:
        if _detect_synopsys_packages(files, repo_root):
            ...
            print_green(
                "[GHDL-INCREMENTAL] Added -fsynopsys flag for Synopsys package support"
            )
...
def compile_incremental(
    ...
    print_green(
        f"[GHDL-INCREMENTAL] Starting incremental compilation for top entity: {top_entity}"
    )
    ...
    try:
        ...
        if vendor_files:
            print_red(
                f"[GHDL-INCREMENTAL] ✗ Top entity file uses vendor-specific libraries, cannot compile with GHDL"
            )
            return (
                1,
                "Top entity file contains vendor-specific libraries incompatible with GHDL",
                current_files,
            )
        ...
        for iteration in range(1, max_iterations + 1):
            print_blue(
                f"[GHDL-INCREMENTAL] Iteration {iteration}/{max_iterations} | files={len(current_files)}"
            )
            ...
            if rc == 0:
                ...
                print_blue(
                    f"[GHDL-INCREMENTAL] Analysis succeeded, running elaboration..."
                )
                elab_cmd = _build_elab_cmd(
                    top_entity,
                    workdir,
                    ghdl_extra_flags,
                    work_library,
                ...
                if rc_elab == 0:
                    print_green(
                        f"[GHDL-INCREMENTAL] ✓ Elaboration successful after {iteration} iterations!"
                    )
                    return rc_elab, output + "\n" + output_elab, current_files

                # Elaboration failed - parse errors from elaboration output
                print_yellow(
                    f"[GHDL-INCREMENTAL] Elaboration failed, need to add more dependencies..."
                )
                output = (
                    output + "\n" + output_elab
                )  # Combine both outputs for error parsing
            ...
            if reordered_files != ordered_files:
                ...
                _ghdl_clean_work(repo_root, workdir, work_library)
                cmd = _build_ghdl_cmd(
                    reordered_files, top_entity, workdir, ghdl_extra_flags, work_library
                )
                rc, output = _run(cmd, repo_root, timeout)
                ...
                if rc == 0:
                    ...
                    print_blue(
                        f"[GHDL-INCREMENTAL] Analysis succeeded after reordering, running elaboration..."
                    )
                    elab_cmd = _build_elab_cmd(
                        top_entity, workdir, ghdl_extra_flags, work_library
                    )
                    rc_elab, output_elab = _run(elab_cmd, repo_root, timeout)

                    if rc_elab == 0:
                        print_green(
                            f"[GHDL-INCREMENTAL] ✓ Elaboration successful after reordering!"
                        )
                        return rc_elab, output + "\n" + output_elab, current_files

                    # Elaboration failed - parse errors
                    print_yellow(
                        f"[GHDL-INCREMENTAL] Elaboration failed after reordering..."
                    )
                    output = output + "\n" + output_elab
            ...
            missing_packages = [
                p for p in missing_packages if p.lower() not in entity_names_lower
            ]

            print_blue(
                f"[GHDL-INCREMENTAL] Missing: entities={len(missing_entities)} packages={len(missing_packages)}"
            )
            ...
            for pkg_name in missing_packages:
                pkg_files = _find_file_declaring_package(
                    repo_root, pkg_name, modules, repo_name
                )
                ...
                if pkg_files:
                    for pkg_file in pkg_files:
                        if pkg_file not in added_files_history:
                            ...
                            vendor_files = _detect_vendor_libraries(
                                [pkg_file], repo_root
                            )
                            if vendor_files:
                                print_yellow(
                                    f"[GHDL-INCREMENTAL] Skipping {pkg_file} due to vendor-specific libraries"
                                )
                                continue
                            ...
                            print_green(
                                f"[GHDL-INCREMENTAL] + Adding package file: {pkg_file} (provides '{pkg_name}')"
                            )
            ...
            for entity_name in missing_entities:
                entity_files = _find_file_declaring_entity(
                    repo_root, entity_name, modules, repo_name
                )
                ...
                if entity_files:
                    for entity_file in entity_files:
                        if entity_file not in added_files_history:
                            # Check for vendor-specific libraries before adding
                            vendor_files = _detect_vendor_libraries(
                                [entity_file], repo_root
                            )
                            if vendor_files:
                                print_yellow(
                                    f"[GHDL-INCREMENTAL] Skipping {entity_file} due to vendor-specific libraries"
                                )
                                continue
                            ...
                            print_green(
                                f"[GHDL-INCREMENTAL] + Adding entity file: {entity_file} (provides '{entity_name}')"
                            )
                            added_something = True
                            break  # Use first candidate
            ...
            if added_something:
                files_are_dependency_ordered = False
                print_yellow(
                    f"[GHDL-INCREMENTAL] Added new files, will reorder in next iteration"
                )
                print_blue(
                    f"[GHDL-INCREMENTAL] Current file order: {', '.join(current_files)}"
                )

                # Immediately try reordering to fix dependency issues
                reordered_files = _reorder_by_dependencies(
                    current_files, output, repo_root
                )
                if reordered_files != current_files:
                    print_yellow(
                        f"[GHDL-INCREMENTAL] Reordering files after adding dependencies..."
                    )
                    print_blue(
                        f"[GHDL-INCREMENTAL] New file order: {', '.join(reordered_files)}"
                    )
                    current_files = reordered_files
                    files_are_dependency_ordered = True
                else:
                    ...
                    if basic_ordered != current_files:
                        print_yellow(
                            f"[GHDL-INCREMENTAL] Applying basic VHDL ordering..."
                        )
                        print_blue(
                            f"[GHDL-INCREMENTAL] Basic ordered: {', '.join(basic_ordered)}"
                        )
            ...
            if not added_something:
                print_red(
                    f"[GHDL-INCREMENTAL] ✗ No progress made in iteration {iteration}"
                )
                ...
                if iteration > 2 and len(current_files) > 5:
                    print_yellow(
                        "[GHDL-INCREMENTAL] Attempting fallback: including ALL VHDL files..."
                    )
                    all_vhdl_files = []
                    for mod_name, file_path in modules:
                        normalized_path = (
                            _normalize_file_path(file_path, repo_name)
                            if repo_name
                            else file_path
                        )
                        if (
                            normalized_path.lower().endswith((".vhd", ".vhdl"))
                            and normalized_path not in added_files_history
                        ):
                            # Check for vendor-specific libraries before adding
                            vendor_files = _detect_vendor_libraries(
                            ...
                            if vendor_files:
                                print_yellow(
                                    f"[GHDL-INCREMENTAL] Skipping {normalized_path} in fallback due to vendor-specific libraries"
                                )
                                continue
                    ...
                    if all_vhdl_files:
                        ...
                        print_green(
                            f"[GHDL-INCREMENTAL] Added {len(all_vhdl_files)} additional VHDL files for fallback"
                        )
        ...
        print_red(
            f"[GHDL-INCREMENTAL] ✗ Failed to achieve clean compilation after {max_iterations} iterations"
        )
...
def incremental_compilation(
    ...
    """
    if context is not None and context.top_module_override is not None:
        top_module_override = context.top_module_override

    print_green(
        f"[GHDL-INCREMENTAL] Trying incremental bottom-up approach for {repo_name}"
    )
    print_blue(
        f"[GHDL-INCREMENTAL] Candidates to try: {', '.join(top_candidates[:10])}"
    )
    ...
    if len(top_candidates) > MAX_CANDIDATES:
        print_yellow(
            f"[GHDL-INCREMENTAL] Limiting to top {MAX_CANDIDATES} candidates (out of {len(top_candidates)})"
        )
        top_candidates = top_candidates[:MAX_CANDIDATES]

    for idx, candidate in enumerate(top_candidates, 1):
        print_blue(
            f"[GHDL-INCREMENTAL] === Candidate {idx}/{len(top_candidates)}: {candidate} ==="
        )
        print_green(f"[GHDL-INCREMENTAL] Testing top entity: {candidate}")

        # Find the file that contains this entity
        entity_files = _find_file_declaring_entity(
            repo_root, candidate, modules, repo_name
        )

        if not entity_files:
            print_yellow(
                f"[GHDL-INCREMENTAL] Could not find file for entity '{candidate}'"
            )
            continue
        ...
        if top_entity_file.startswith(f"temp/{repo_name}/"):
            ...
            print_blue(
                f"[GHDL-INCREMENTAL] Adjusted top entity file: {top_entity_file}"
            )

        # Check if file exists
        full_path = (
            os.path.join(repo_root, top_entity_file)
            if not os.path.isabs(top_entity_file)
            else top_entity_file
        )
        ...
        if (
            top_module_override
            and candidate == top_module_override
