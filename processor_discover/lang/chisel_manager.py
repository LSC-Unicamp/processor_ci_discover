...
def _is_interface_module_name(name: str) -> bool:
    return is_interface_module_name(name, suffixes=("if",))
...
def find_top_module(
    ...
    for module in module_graph.keys():
        ...
        if num_parents <= 3 and any(
            pat in name_lower for pat in ["core", "cpu", "processor", "riscv"]
        ):
            if not any(
                bad in name_lower
                for bad in ["test", "tb", "bench", "periph", "uart", "spi", "gpio"]
            ):
    ...
    for c in candidates:
        reach = _reachable_size(
            module_graph, c
        )  # How many modules does this instantiate
        ...
        if name_lower == "top" and repo_lower:
            repo_name_exists = any(
                repo_lower == mod.lower() for mod in module_graph.keys()
            )
        ...
        if any(
            pat in name_lower
            for pat in ["crypto", "aes", "sha", "rsa", "nist", "cipher"]
        ):
            score -= 25000

        # Crossing/bridge penalty - infrastructure modules
        if (
            any(pat in name_lower for pat in ["xing", "crossing", "mute", "rational"])
            and "core" not in name_lower
        ):
            score -= 20000
        ...
        if any(
            pat in name_lower
            for pat in ["sourcenode", "sinknode", "tomodule", "tobundle"]
        ):
            score -= 25000
...
def find_all_main_apps(
    directory: str, top_module: str, hdl_type: str = "chisel", repo_name: str = None
    ...
    for scala_file in scala_files:
        try:
            ...
            main_method_match = re.search(
                r"object\s+(\w+)\s*\{[^}]*def\s+main\s*\(\s*args\s*:\s*Array\[String\]\s*\)",
                content,
                re.DOTALL,
            )
            ...
            if app_match:
                ...
                requires_args = bool(
                    re.search(
                        r"args\s*[\(\.\[]", content[main_start : main_start + 2000]
                    )
                )
            else:
                continue
            ...
            if hdl_type == "spinalhdl":
                if "SpinalVerilog" in content or "SpinalConfig" in content:
                    ...
                    spinal_block_pattern = re.search(
                        r"Spinal(?:Verilog|Config)[^\{]*\{[^\{]*?new\s+(\w+)\s*[(\[]",
                        content,
                        re.DOTALL,
                    )
                    ...
                    if spinal_pos > 0:
                        ...
                        val_pattern = re.search(
                            r"val\s+\w+\s*=\s*new\s+(\w+)\s*[(\[]", after_spinal
                        )
                        if val_pattern:
                            ...
                            for module_name in all_news:
                                if (
                                    module_name not in plugin_names
                                    and not module_name.endswith("Config")
                                ):
                        ...
                        module_instantiation = re.search(
                            r"new\s+(\w+)\s*[(\[]", content
                        )
                    ...
                    if any(
                        periph in instantiated_module_lower
                        for periph in peripheral_names
                    ):
                        score -= 20000
                    ...
                    if repo_lower and len(repo_lower) > 2:
                        ...
                        if (
                            repo_lower == filename_normalized
                            or repo_lower == app_normalized
                        ):
                            score += 10000
                        elif (
                            repo_lower in filename_normalized
                            or repo_lower in app_normalized
                        ):
                            score += 8000
                    ...
                    if "wishbone" in filename_lower or "wishbone" in app_name_lower:
                        score += 20000
                    if (
                        "wb" in filename_lower
                        or "_wb" in app_name_lower
                        or "wb_" in app_name_lower
                    ):
                    ...
                    soc_count = sum(
                        1 for indicator in soc_indicators if indicator in content_lower
                    )
                    ...
                    if any(
                        soc in filename_lower or soc in app_name_lower
                        for soc in known_socs
                    ):
                        score -= 3000
                    ...
                    candidates.append(
                        (score, scala_file, main_class, app_name, instantiated_module)
                    )
            ...
            # For Chisel, look for ChiselStage or emitVerilog
                ...
                if "ChiselStage" in content or "emitVerilog" in content:
                    ...
                    if repo_lower and len(repo_lower) > 2:
                        ...
                        if (
                            repo_lower == filename_normalized
                            or repo_lower == app_name_lower
                        ):
                            score += 10000
                        elif (
                            repo_lower in filename_normalized
                            or repo_lower in app_name_lower
                        ):
                            score += 8000
    ...
    for idx, (score, file, main_class, app_name, inst_module) in enumerate(
        candidates[:10]
    ):  # Show top 10
...
def find_existing_main_app(
    ...
    if best_match[0] < 0:
        print(
            f"[WARNING] Best App candidate requires arguments (score: {best_match[0]})"
        )
        ...
        for candidate in candidates:
            if candidate[0] > 0:
                ...
                print(
                    f"[INFO] Found alternative App without arguments: {candidate[3]} (score: {candidate[0]})"
                )
                break
        else:
            # No candidates with positive score - return None to generate our own
            print(
                f"[WARNING] All App candidates require arguments - will generate new main App"
            )
            return None
    ...
    if len(candidates) > 1:
        ...
        for score, file, main_class, app_name, inst_module in candidates[
            1 : min(5, len(candidates))
        ]:
            print(
                f"  - {app_name} -> {inst_module} ({os.path.basename(file)}) - score: {score}"
            )
...
def find_build_file(
    directory: str, top_module: str = None, modules: List[Tuple[str, str]] = None
                ...
                try:
                    with open(build_file, "r", encoding="utf-8", errors="ignore") as f:
                        ...
                        if top_module in content:
                            print(
                                f"[INFO] Found build.sbt referencing top module: {build_file}"
                            )
            ...
            try:
                with open(build_file, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    if "chisel" in content.lower():
                        print(
                            f"[INFO] Found build.sbt with Chisel dependencies: {build_file}"
                        )
                        return (build_file, "sbt")
            except Exception:
                continue
        ...
        if os.path.exists(build_sc):
            try:
                ...
                module_matches = re.findall(
                    r"object\s+(\w+)\s+extends\s+(?:\w+(?:Module|NS))", content
                )
                if module_matches:
...
def process_chisel_project(directory: str, repo_name: str = None) -> Dict:
    ...
    if app_candidates and len(app_candidates) > 0:
        ...
        for idx, (
            ...
            app_name,
            instantiated_module,
        ) in enumerate(app_candidates):
            print(
                f"[INFO] Trying App {idx+1}/{len(app_candidates)}: {app_name} (score: {score}, instantiates: {instantiated_module})"
            )

            # Try to run this App - use build_directory instead of directory
            success, verilog_file, log = emit_verilog(
                build_directory,
                app_path,
                main_class_override=main_class,
    ...
    if not success:
        ...
        success, verilog_file, log = emit_verilog(
            build_directory, main_app, build_tool=build_tool
        )
