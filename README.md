...
- `processor_ci_discover/`: package root
- `processor_ci_discover/core/`: generator and core helpers
- `processor_ci_discover/lang/`: Chisel/Bluespec helpers
- `processor_ci_discover/runners/`: simulator/minimization backends
- `processor_ci_discover/utils/`: shared utilities (logging, locks)
- `processor_ci_discover/cli.py`: CLI parsing and package entrypoint
...
```bash
python -m processor_ci_discover.cli -u <processor_repo_url> -p config/
```
...
```bash
python -m processor_ci_discover.cli -u https://github.com/<org>/<repo> -p config/
```
...
```bash
python -m processor_ci_discover.cli \
  -u https://github.com/<org>/<repo> \
...
```text
usage: python -m processor_ci_discover.cli [-h] -u PROCESSOR_URL
                                           [-p CONFIG_PATH] [-g] [-a] [-n]
...
- `processor_ci_discover.cli.build_parser()` exposes the CLI parser for tests.
- `processor_ci_discover.cli.main(argv=None)` supports argument injection for tests and wrappers.
- `processor_ci_discover.core.config_generator.generate_processor_config(...)` remains the stable programmatic entrypoint.
- `processor_ci_discover.core.config_generator.main(argv=None)` is a compatibility shim into the package CLI.

## Notes

- The current migration keeps CLI behavior stable while moving parser ownership into `processor_ci_discover/cli.py`.
